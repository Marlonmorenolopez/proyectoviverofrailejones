...
var onCronTrigger = (runtime2) => {
  runtime2.log(`?? Chainlink CRE consultando clima de Bogot�...`);
  try {
    ...
    const request = client.sendRequest(runtime2, { url, method: "GET" });
    const response = request.result();
    runtime2.log(`?? RESPUESTA CRUDA: ${JSON.stringify(response).substring(0, 150)}...`);
    let data;
    if (response.body) {
...
? �DATOS DECIFRADOS CON EXITO!`);
    runtime2.log(`??? Temperatura detectada: ${data.main.temp} C`);
    runtime2.log(`?? Humedad detectada: ${humedad} %`);
    ...
    runtime2.log(`? Error fatal en el Or�culo: ${error.message}`);
    throw error;
  }
};
